...
namespace TradingLib.MarketData
{
    ...
    /// </summary>
    public class MDTradeTickWriter : BinaryWriter
    {
        const string FILE_DOT_EXT = ".tick";
        ...
        public MDTradeTickWriter(MDSymbol symbol, int date)
        {
            _symbol = symbol.Symbol;
            _path = GetTickPath(symbol.Exchange, symbol.SecCode);
            _date = date;
        ...
        /// <param name="symbol"></param>
        /// <returns></returns>
        public static string GetTickPath(string exchange,string secCode)
        {
            string path = Path.Combine(new string[] { AppDomain.CurrentDomain.BaseDirectory,"Data","Tick", exchange, secCode });
        ...
        public void NewTick(long datetime, double price, int size, int vol)
        {
