...
namespace TradingLib.MarketData
{
    public class Profiler
    {
        ...
        public List<string> GetStatsStringList()
        {
            ...
            list.Add("Name\t\tTimes Called\tLocal Time\tTotal Time\tAVG Time");
            foreach (SectionStats stats in this.__sectionStatslist.Values)
            {
                list.Add(string.Format("{0}\t\t{1}\t{2:f5}\t\t{3:f5}\t\t{4}ms\r\n", new object[] { stats.Name, stats.TimesCalled, stats.LocalTime.TotalSeconds, stats.TotalTime.TotalSeconds,(int)stats.TotalTime.TotalMilliseconds/stats.TimesCalled }));
            }
            return list;
        }
