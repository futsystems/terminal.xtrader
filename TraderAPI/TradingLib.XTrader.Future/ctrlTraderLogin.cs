    ...
    public partial class ctrlTraderLogin : UserControl
    {
        ...
        ctrlFutureTrader CreateTraderControl()
        {
            if (InvokeRequired)
            {
                ...
                return Invoke(new Func<ctrlFutureTrader>(CreateTraderControl), new object[] { }) as ctrlFutureTrader;
;
            }
            else
            {
                ctrlFutureTrader tmp = new ctrlFutureTrader();
        ...
        void Connect()
        {
            ...
            //logger.Info("client try to connec to:" + node.Address + " port:" + node.Port.ToString());
            CoreService.InitClient(TradingLib.XTrader.Future.Constants.AppServer, 41055);
