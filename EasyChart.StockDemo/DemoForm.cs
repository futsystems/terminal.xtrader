...
namespace WindowsDemo
{
	...
	{
        ILog logger = LogManager.GetLogger("DemoForm");
        ...
        TradingLib.MDClient.MDClient client = null;
		...
		public DemoForm()
		{
            ...
            ChartControl.BeforeCycleChange += new EventHandler(ChartControl_BeforeCycleChange);
        ...
        {
            //logger.Info("??????after data bind ,{0} {1}".Put(e.cdp.ToString(), e.dmb.ToString()));
            //logger.Info("datacycle:{0} symbol:{0}".Put(e.cdp.DataCycle, e.cdp.GetStringData("Code")));

        }
        TLMemoryDataManager tlmdm = null;
        void ChartControl_MouseClick(object sender, MouseEventArgs e)
        {
            //logger.Info("mouse clicek");
            if (tlmdm != null)
            { 
                //
                //logger.Info("~~~~~~~~~~~~~~~~~~~~~~");
        ...
        void ChartControl_AfterCycleChange(object sender, EventArgs e)
        {

            logger.Info("after cycle change");
        }

        void ChartControl_BeforeCycleChange(object sender, EventArgs e)
        {
            
            logger.Info("before cycle change");
        }
		...
		private void InitializeComponent()
		{
			...
			this.miLoadFromYahooStreaming.Text = "TL-MDClient";
		...
		private void miLoadBinary_Click(object sender, System.EventArgs e)
		{
            ...
            //ChartControl.EndTime = DateTime.MinValue;
            logger.Info("Use MDRemoteDataManager");
            ChartControl.HistoryDataManager = new MDRemoteDataManager();
		...
		private void miLoadFromYahooStreaming_Click(object sender, System.EventArgs e)
		{
            ...
            //YahooDataManager ydm = new YahooDataManager();
			//ydm.CacheRoot = FormulaHelper.Root+"Cache";
            //string filename = Environment.CurrentDirectory + "\\IF1604.CSV";
            //DataManagerBase dmb = new YahooCSVDataManager(Path.GetDirectoryName(filename), Path.GetExtension(filename));
            ...
            //return;
            //TLLocalDataManager dmd = new TLLocalDataManager();
            ...
            tlmdm = new TLMemoryDataManager(true, client);
            tlmdm.DataProcessedEvent += new Action(mdm_DataProcessedEvent);
            tlmdm.BarUpdatedEvent += new Action(tlmdm_BarUpdatedEvent);
            
				
			//Assign memory data manager to the chart control.
			//string s = LoadDataManager(mdm,"IF1604",DataCycle.Day,200);
            ...
            //    (dcbStream as TLDataClient).QryBar();
            //} 
            //else  
            //{
        ...
        void mdm_DataProcessedEvent()
        {
            
            //ChartControl.BindData();
            ChartControl.NeedRebind();
            //ChartControl.NeedRedraw();
            //ChartControl.NeedRefresh();ChartControl.StockBars = 200;
            
            //ChartControl.EndTime = DateTime.MaxValue;
            logger.Info("ctl start:{0} end:{1}".Put(ChartControl.StartTime, ChartControl.EndTime));
        }
...
//			else dcbForex.SetStreamingSymbol(s);
		...
		private void dcb_OnStreamingData(object sender, DataPacket dp)
		{
            if (InvokeRequired)
            {
                Invoke(new Action<object, DataPacket>(dcb_OnStreamingData), new object[] { sender, dp });
            }
            else
            {
